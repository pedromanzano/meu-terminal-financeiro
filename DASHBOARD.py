...
with st.sidebar:
    ...
    if st.session_state.get("authentication_status"):
        ...
        with st.form("form_nova_transacao", clear_on_submit=True):
            if f_mercado == "Renda Fixa": f_ativo = st.text_input("Nome do Título")
            else: f_ativo = st.selectbox("Ativo (Ex: PETR4, BTC)", options=lista_acoes_atual)
            ...
            if st.form_submit_button("Salvar") and f_ativo:
                supabase.table("transacoes").insert({"usuario": st.session_state["username"], "ativo": f_ativo.strip(), "mercado": f_mercado, "tipo": f_tipo, "quantidade": f_qtd, "preco": f_preco}).execute()
        ...
        if arquivo_import is not None and st.button("🚀 Processar e Salvar"):
            try:
                with st.spinner("Lendo o extrato..."):
                    ...
                    col_p = 'preço' if 'preço' in df_import.columns else 'preco'
                    
                    if 'ativo' in df_import.columns and 'quantidade' in df_import.columns and col_p in df_import.columns:
                        novas_ordens =[]
                        for _, linha in df_import.iterrows():
                            ...
                            tipo_ordem = "Compra" if str(linha.get('entrada/saída', '')).strip().lower() in['credito', 'entrada', 'c', 'compra'] else "Venda"
                            mercado = "Criptomoedas" if any(crip in ativo_nome for crip in['BTC', 'ETH', 'SOL']) else ("Renda Fixa" if any(rf in ativo_nome for rf in ['TESOURO', 'CDB', 'LCI', 'LCA']) else "B3 (Ações/FIIs)")

                            try:
                                if float(linha['quantidade']) > 0:
                                    novas_ordens.append({"usuario": st.session_state["username"], "ativo": ativo_nome, "mercado": mercado, "tipo": tipo_ordem, "quantidade": float(linha['quantidade']), "preco": float(linha[col_p])})
                            except: continue
                        if novas_ordens:
...
# ==========================================
    ...
    if not st.session_state.get("authentication_status"):
        ...
        try: authenticator.login(location='main', key='login_carteira')
        except TypeError: authenticator.login("main")
        ...
        try:
                ...
                if df_base.empty:
                    ...
                    if not df_b3_raw.empty:
                        ...
                        if not carteira.empty:
                            ...
                            # O lucro Não Realizado apenas dos ativos atuais
                        ...
                        # Lucro Total Global = Lucro que já foi pro bolso + Oscilação das ações que tem hoje
                        lucro_b3_reais = lucro_realizado_b3 + (patrimonio_b3 - investido_b3)

                    # ==============================================
                    # 2. MOTOR DE CUSTÓDIA PARA CRIPTO
                    # ==============================================
                    ...
                    with st.spinner("Buscando dados da B3..."):
                        ...
                        if not dados_h.empty:
                            ...
                            if not df_cdi.empty:
                                ...
                                if getattr(df_comp.index, 'tz', None) is not None: df_comp.index = df_comp.index.tz_localize(None)
                                if getattr(df_cdi.index, 'tz', None) is not None: df_cdi.index = df_cdi.index.tz_localize(None)
                ...
                if st.button("🚨 Iniciar Simulação") and not carteira.empty:
                    ...
                    with st.spinner(f"Viajando no tempo para {data_inicio}..."):
                        ...
                        if not dados_crise.empty and '^BVSP' in dados_crise.columns:
                            ativos_validos = dados_crise.iloc[0][dados_crise.iloc[0] > 0].index.tolist()
                            ...
                            if ativos_carteira_validos:
                                ...
                                df_stress = pd.DataFrame({'A Minha Carteira': (retornos_crise[ativos_carteira_validos] * pesos_stress).sum(axis=1) * 100, 'Ibovespa': retornos_crise['^BVSP'] * 100})
            ...
            with tab5:
                ...
                if not carteira.empty:
                    ...
                    if pesos_atuais: pesos_atuais[0] = round(pesos_atuais[0] + (100.0 - sum(pesos_atuais)), 2)
                    
                    df_alvos = pd.DataFrame({'Ativo': ativos_carteira_limpos, 'Alvo (%)': pesos_atuais})
                    alvos_editados = st.data_editor(df_alvos, num_rows="dynamic", use_container_width=True, column_config={"Ativo": st.column_config.SelectboxColumn("Ativo", options=buscar_tickers_brapi(), required=True), "Alvo (%)": st.column_config.NumberColumn("Alvo (%)", min_value=0.0, max_value=100.0, format="%.2f", step=0.5)})
                    
                    soma_alvos = alvos_editados['Alvo (%)'].sum()
                    diferenca = 100.0 - soma_alvos
                    
                    if abs(diferenca) < 0.01:
                        st.success(f"✅ Soma Perfeita. Pode gerar ordens.")
                        if st.button("🚀 Gerar Ordens"):
                            pat_futuro = patrimonio_b3 + novo_aporte
                            ordens =[]
                            for _, row in alvos_editados.iterrows():
                                a_limpo, a_pct = str(row['Ativo']).upper().strip(), row['Alvo (%)'] / 100.0
                                a_sa = a_limpo + '.SA'
                                v_at = carteira.loc[carteira['ativo'] == a_sa, 'valor_patrimonio_atual'].values[0] if a_sa in carteira['ativo'].values else 0.0
                                p_at = carteira.loc[carteira['ativo'] == a_sa, 'preco_atual'].values[0] if a_sa in carteira['ativo'].values else obter_preco_atual(a_limpo)
                                
                                qtd = int((pat_futuro * a_pct - v_at) / p_at) if p_at > 0 else 0
                                if qtd != 0: ordens.append({'Operação': "COMPRAR" if qtd > 0 else "VENDER", 'Ativo': a_limpo, 'Qtd': abs(qtd), 'Preço Base': p_at, 'Total (R$)': abs(qtd * p_at)})
            ...
            with tab7:
                st.subheader("🎯 Radar de Distorções Imobiliárias")
                fiis_config = {'KNCR11': 'Papel', 'KNIP11': 'Papel', 'MXRF11': 'Papel', 'CPTS11': 'Papel', 'HGLG11': 'Logística', 'BTLG11': 'Logística', 'XPLG11': 'Logística', 'HGRU11': 'Híbrido/Urbano', 'KNRI11': 'Híbrido', 'HGRE11': 'Lajes', 'XPML11': 'Shoppings'}
                with st.spinner("Sincronizando métricas..."):
                    df_radar = buscar_pvp_fiis(fiis_config)
                    if not df_radar.empty:
                        secoes = {"📄 Papel": "Papel", "🚛 Logística": "Logística", "🏢 Lajes": "Lajes", "🛍️ Híbrido/Shoppings":["Shoppings", "Híbrido", "Híbrido/Urbano"]}
                        for titulo, filtro in secoes.items():
                            ...
                            if not df_setor.empty: st.dataframe(df_setor.style.apply(style_pvp_inteligente, axis=1).format({'Preço': 'R$ {:.2f}', 'VPA': 'R$ {:.2f}', 'P/VP': '{:.2f}', 'DY Anual (%)': '{:.2f}%', 'DY Mensal Est. (%)': '{:.2f}%'}), use_container_width=True, hide_index=True)
                    else: st.error("Falha na comunicação com a B3.")
