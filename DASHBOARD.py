...
def obter_preco_atual(ticker):
    ...
    try:
        ...
        # Fallback YFinance se BRAPI falhar (Corrigido para evitar o fast_info depreciado)
        try: 
    ...
    try:
        ...
        if not divs.empty:
            if getattr(divs.index, 'tz', None) is not None:
                divs.index = divs.index.tz_localize(None) # ⏰ A MÁGICA SEGURA: Arranca o fuso horário
            return divs
    except: pass
    
    # 2. Se o YFinance falhar, a BRAPI entra como plano de backup
    try:
        ...
        if divs_list:
            ...
            if getattr(df['date'].dt, 'tz', None) is not None:
                df['date'] = df['date'].dt.tz_localize(None) # ⏰ Sem fuso!
...
def carregar_dados_historicos(tickers, start=None, end=None, period="1y"):
    ...
    try:
        ...
        if isinstance(df, pd.Series): 
            ...
            # O Yahoo Finance embaralha as colunas em ordem alfabética. Mapeamos com segurança:
            mapa_nomes = {yf_ticker: original for yf_ticker, original in zip(tickers_yf, tickers)}
            df.rename(columns=mapa_nomes, inplace=True)
        
        # Preenche buracos normais (feriados), mas deixa em branco onde a ação não existia
        return df.ffill()
...
def carregar_cdi_historico(data_inicio, data_fim):
    """Busca o CDI direto do Banco Central"""
    try:
...
def buscar_pvp_fiis(lista_fiis_config):
    ...
    for fii, segmento in lista_fiis_config.items():
        try:
            ...
            if pvp and pvp > 0:
                # --- NOVA LÓGICA DE STATUS TEXTUAL ---
                if segmento == 'Papel':
                    if 0.98 <= pvp <= 1.02:
                        status_texto = "🟢 Paridade"
                    elif pvp < 0.98:
                        status_texto = "🟠 Alerta: Risco" # VRTA11 vai cair aqui
                    else:
                        status_texto = "🔴 Ágio"
                else:
                    # Lógica para Tijolo
...
def scanner_volume_atipico():
    ...
    try:
        # Puxamos 5 dias para garantir que o Yahoo tenha dados de hoje (intervalo 5m é mais estável)
...
def style_pvp_inteligente(row):
    status = row['Status']
    if "🟢" in status:
        return['color: #00CC96; font-weight: bold'] * len(row)
    elif "🟠" in status:
        return['color: #FFA500; font-weight: bold'] * len(row)
    else:
        return ['color: #EF553B; font-weight: bold'] * len(row)
...
credentials = {
    "usernames": {
        "admin": { "name": "Pedro Admin", "password": "$2b$12$eaa2xg5WmNGWTfJL8Y09buVjlBAThqlMwsogcnyYfUFodd3VZt6jO" }
    }
}
...
with st.sidebar:
    ...
    if st.session_state.get("authentication_status"):
        ...
        with st.form("form_nova_transacao", clear_on_submit=True):
            ...
            with col_f1:
                f_qtd = st.number_input("Quantidade", min_value=0.0001, format="%.4f", step=1.0)
            with col_f2:
                f_preco = st.number_input("Preço Unitário", min_value=0.01, step=10.0)
        ...
        if not api_key:
            st.error("Chave da API Google Generative não configurada em secrets.toml.")
            st.stop()
            
        genai.configure(api_key=api_key)
        # Corrigido para a versão oficial de produção da API
            ...
            with chat_container.chat_message("assistant"):
                with st.spinner("Consultando dados..."):
                    ...
                    try:
                        ...
                        st.error("Não foi possível gerar a resposta da I.A. no momento.")
...
if aba_selecionada == "🏠 Home (Mercado)":
    ...
    for i, (ticker, nome) in enumerate(indices.items()):
        ...
        if len(data) > 1:
            ...
            variacao = ((valor / data['Close'].iloc[-2]) - 1) * 100
            cols[i].metric(nome, f"{valor:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."), f"{variacao:+.2f}%")
    ...
    with col_s1:
        ...
        st.caption("O índice analisa a volatilidade implícita vs. média histórica para medir o apetite ao risco.")

    with col_s2:
        st.write("### 📈 Fluxo Institucional (Smart Money)")
        st.caption("Detecção de anomalias via RVOL (Volume Proporcional em tempo real).")
...
# ==========================================
    ...
    if not st.session_state.get("authentication_status"):
        ...
        try:
            ...
            # Compatibilidade com a nova versão do streamlit_authenticator
            authenticator.login("main")
    
    if st.session_state.get("authentication_status"):
        try:
            with st.spinner("Sincronizando com o cofre digital..."):
                resposta = supabase.table("transacoes").select("*").eq("usuario", st.session_state["username"]).execute()
                df_base = pd.DataFrame(resposta.data)

                patrimonio_b3, investido_b3, lucro_b3_reais, rentabilidade_b3_pct = 0.0, 0.0, 0.0, 0.0
                valor_criptos_total, investido_criptos_total, saldo_renda_fixa = 0.0, 0.0, 0.0
                ...
                if df_base.empty:
                    ...
                    if not df_b3_raw.empty:
                        ...
                        df_b3_raw['qtd_adj'] = np.where(df_b3_raw['tipo'] == 'Compra', df_b3_raw['quantidade'], -df_b3_raw['quantidade'])
                        
                        compras = df_b3_raw[df_b3_raw['qtd_adj'] > 0].copy()
                        custo_compra = compras.groupby('ativo_limpo').apply(lambda x: (x['quantidade'] * x['preco']).sum() / x['quantidade'].sum()).reset_index(name='preco_medio')
                        
                        posicao = df_b3_raw.groupby('ativo_limpo')['qtd_adj'].sum().reset_index()
                        carteira = pd.merge(posicao[posicao['qtd_adj'] > 0], custo_compra, on='ativo_limpo', how='left').rename(columns={'ativo_limpo': 'ativo', 'qtd_adj': 'quantidade_total'})
                        ...
                        for _, row in carteira.iterrows():
                            ...
                            try:
                                ...
                                if not hist_div.empty:
                                    if getattr(hist_div.index, 'tz', None) is not None:
                    ...
                    if not df_c_raw.empty:
                        df_c_raw['qtd_adj'] = np.where(df_c_raw['tipo'] == 'Compra', df_c_raw['quantidade'], -df_c_raw['quantidade'])
                        compras_c = df_c_raw[df_c_raw['qtd_adj'] > 0]
                        pm_c = compras_c.groupby('ativo').apply(lambda x: (x['quantidade'] * x['preco']).sum() / x['quantidade'].sum()).reset_index(name='pm')
                        
                        df_cripto = pd.merge(df_c_raw.groupby('ativo')['qtd_adj'].sum().reset_index(), pm_c, on='ativo', how='left')
                        df_cripto = df_cripto[df_cripto['qtd_adj'] > 0].rename(columns={'ativo': 'Ativo Cripto', 'qtd_adj': 'Quantidade', 'pm': 'Preço Médio'})
                        ...
                        df_cripto['Rentabilidade (%)'] = (df_cripto['Lucro/Prejuízo (R$)'] / (df_cripto['Quantidade'] * df_cripto['Preço Médio'])) * 100
                    ...
                    if not df_rf_raw.empty:
                        df_rf = df_rf_raw.groupby('ativo')['preco'].sum().reset_index().rename(columns={'ativo': 'Ativo RF', 'preco': 'Valor Atual'})
            ...
            c2.metric("📈 Lucro/Prejuízo (B3)", f"R$ {lucro_b3_reais:,.2f}", f"{rentabilidade_b3_pct:.2f}%" if investido_b3 > 0 else None)
            c3.metric("🏦 Renda Fixa", f"R$ {saldo_renda_fixa:,.2f}")
            c4.metric("₿ Criptomoedas", f"R$ {valor_criptos_total:,.2f}", f"{((valor_criptos_total/investido_criptos_total)-1)*100:.2f}%" if investido_criptos_total > 0 else None)
            ...
            tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs([
                "📊 Visão Geral", "💸 Renda Passiva", "🛡️ Risco e Defesa", 
                "🧠 Inteligência", "⚙️ Gestão", "💎 Outros Ativos", "🎯 Radar de Oportunidades"
            ])
            ...
            with tab1:
                ...
                if not carteira.empty:
                    ...
                    # CORREÇÃO: regex=False para não corromper nomes como B3SA3 ou ITSA4
                    df_view['ativo'] = df_view['ativo'].str.replace('.SA', '', regex=False).str.upper()
                ...
                if not carteira.empty:
                    with st.spinner("Buscando dados da B3 e do Banco Central..."):
                        ...
                        if not dados_h.empty:
                            ...
                            data_ini_bcb = (pd.Timestamp.now() - pd.DateOffset(years=1)).strftime('%d/%m/%Y')
                            data_fim_bcb = pd.Timestamp.now().strftime('%d/%m/%Y')
                            df_cdi = carregar_cdi_historico(data_ini_bcb, data_fim_bcb)
                            
                            if not df_cdi.empty:
                                df_cdi['Fator'] = 1 + (df_cdi['valor'] / 100)
                                df_cdi['CDI Acumulado'] = (df_cdi['Fator'].cumprod() - 1) * 100
                                
                                # CORREÇÃO: Remoção segura de fuso horário
                                if getattr(df_comp.index, 'tz', None) is not None:
                                    df_comp.index = df_comp.index.tz_localize(None)
                                if getattr(df_cdi.index, 'tz', None) is not None:
            ...
            with tab2:
                ...
                if not carteira.empty:
                    ...
                    if total_divs_detectados > 0:
                        ...
                        # CORREÇÃO: regex=False para manter segurança do nome do ticker
                        df_com_dividendos['ativo'] = df_com_dividendos['ativo'].str.replace('.SA', '', regex=False).str.upper()
                        ...
                        with col_d1:
                            ...
                            st.dataframe(
                                df_com_dividendos[['ativo', 'dividendos_12m', 'projecao_mensal']], 
                                use_container_width=True, hide_index=True, 
                                column_config={
                                    "ativo": "Ticker", 
                                    "dividendos_12m": st.column_config.NumberColumn("Total 12M (R$)", format="R$ %.2f"),
                                    "projecao_mensal": st.column_config.NumberColumn("Média Mensal Estimada", format="R$ %.2f")
                                }
                            )
                        ...
                        with col_d2:
                            ...
                            fig_div_bar = px.bar(
                                df_com_dividendos, x='ativo', y='dividendos_12m', 
                                color_discrete_sequence=['#00CC96'],
                                labels={'ativo': 'Ativo', 'dividendos_12m': 'Proventos (R$)'}
                            )
                            fig_div_bar.update_traces(hovertemplate="<b>%{x}</b><br>Recebido 12M: <b>R$ %{y:,.2f}</b><extra></extra>")
                            fig_div_bar.update_xaxes(title_text="")
                            fig_div_bar.update_yaxes(title_text="")
                    ...
                    with st.spinner(f"Viajando no tempo para {data_inicio}..."):
                        ...
                        if not dados_crise.empty and '^BVSP' in dados_crise.columns:
                            ...
                            if ativos_carteira_validos:
                                ...
                                pesos_stress = pesos_brutos / pesos_brutos.sum()
                                ...
                                fig_stress.update_traces(hovertemplate="<b>%{fullData.name}</b><br>Data: %{x|%d/%m/%Y}<br>Impacto: %{y:.2f}%<extra></extra>")
                                fig_stress.update_xaxes(title_text="Dias após o início da crise")
                                fig_stress.update_yaxes(title_text="")
                                st.plotly_chart(padronizar_grafico(fig_stress), use_container_width=True)
                                
                                st.error(f"⚠️ No pior momento desta crise, a sua carteira teria caído **{df_stress['A Minha Carteira'].min():.2f}%**.")
                                
                                ignorados = len(tickers_stress) - len(ativos_carteira_validos)
                                if ignorados > 0:
                                    st.caption(f"ℹ️ {ignorados} ativo(s) da sua carteira foram ignorados na simulação pois não tinham capital aberto na época da crise.")
                            else:
                                st.warning("Nenhum dos seus ativos atuais tinha capital aberto na época desta crise.")
                        else:
                            st.error("Falha ao se conectar com o histórico da B3 para o período selecionado.")
                ...
                with col_r1:
                    ...
                    if st.button("🧬 Gerar Matriz") and not carteira.empty:
                        ...
                        if not dados_corr.empty:
                            ...
                            # Uso do .replace original pois aqui é nível do python, não Series Pandas
            ...
            with tab4:
                ...
                if st.button("🔍 Calcular Valuation") and not carteira.empty:
                    with st.spinner("Analisando balanços e histórico de dividendos..."):
                        ...
                        if val_list: 
                            st.dataframe(pd.DataFrame(val_list).style.format({'Preço Atual': 'R$ {:.2f}', 'P. Justo (Graham)': 'R$ {:.2f}', 'Margem (Graham)': '{:.2f}%', 'P. Teto (Bazin)': 'R$ {:.2f}'}, na_rep="N/A"), use_container_width=True, hide_index=True)
                ...
                with col_ia1:
                    ...
                    if st.button("🎲 Simular Futuro") and not carteira.empty:
                        ...
                        if not hist.empty:
                            ...
                            fig_mc.update_traces(hovertemplate="<b>%{fullData.name}</b><br>Dia: %{x}<br>Patrimônio: <b>R$ %{y:,.2f}</b><extra></extra>")
                            fig_mc.update_xaxes(title_text="Dias Úteis"); fig_mc.update_yaxes(title_text="")
                            st.plotly_chart(padronizar_grafico(fig_mc), use_container_width=True)
                ...
                with col_ia2:
                    ...
                    if not carteira.empty:
                        ...
                        try:
                            termo_pesquisa = ativo_noticia.replace('.SA', '') + " ações mercado financeiro"
                            url_news = f"https://news.google.com/rss/search?q={urllib.parse.quote(termo_pesquisa)}&hl=pt-BR&gl=BR&ceid=BR:pt-419"
                            ...
                            with urllib.request.urlopen(req, timeout=5) as response:
                                ...
                                for n in root.findall('.//item')[:4]:
                                    ...
                                    st.caption(f"📅 Publicado em: {n.find('pubDate').text[5:16]}")
                                    st.divider()
                        except: st.error("Serviço de notícias indisponível no momento.")
            ...
            with tab5:
                ...
                if not carteira.empty:
                    ...
                    st.markdown("### Status do Rebalanceamento")
                    ...
                    if pode_calcular and st.button("🚀 Gerar Ordens de Compra/Venda"):
                        with st.spinner("Calculando a rota ideal do seu dinheiro..."):
                            ...
                            if ordens:
                                st.dataframe(
                                    pd.DataFrame(ordens).style.map(lambda x: 'color: #00CC96; font-weight: bold' if x=='COMPRAR' else 'color: #EF553B; font-weight: bold', subset=['Operação'])
                                    .format({'Preço Base': 'R$ {:.2f}', 'Total (R$)': 'R$ {:.2f}'}), 
                                    use_container_width=True, 
                                    hide_index=True
                                )
                            else:
                                st.info("Sua carteira já está perfeitamente balanceada! Nenhuma ordem necessária.")
                else: 
                    st.warning("Adicione ativos na B3 para usar o rebalanceamento inteligente.")
            ...
            with tab6:
                ...
                with c_c1:
                    ...
                    if not df_cripto.empty: 
                        st.dataframe(df_cripto, use_container_width=True, hide_index=True, column_config={"Ativo Cripto": "Criptomoeda", "Quantidade": st.column_config.NumberColumn("Qtd", format="%.4f"), "Preço Atual (R$)": st.column_config.NumberColumn("Cotação", format="R$ %.2f"), "Valor Atual (R$)": st.column_config.NumberColumn("Patrimônio", format="R$ %.2f")})
                    else: st.info("Nenhuma cripto cadastrada.")
                with c_c2:
                    st.subheader("🏦 Renda Fixa")
                    if not df_rf.empty: 
                        st.dataframe(df_rf, use_container_width=True, hide_index=True, column_config={"Ativo RF": "Título", "Valor Atual": st.column_config.NumberColumn("Valor Investido", format="R$ %.2f")})
